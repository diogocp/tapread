...
object EmvCdaVerifier {
    ...
    fun verify(tags: CertTags, caKeys: List<CaPublicKey>): CdaVerification {
        ...
        if (tags.iccCertHex.isNullOrBlank() || tags.iccExpHex.isNullOrBlank()) {
            ...
            return done(steps, rid, caIndex, "Cannot verify: ICC certificate data missing.")
        }
        ...
        val icc = try {
            recoverCertKey(
                ...
                expectedFormat = 0x04,
                headerFixedLen = 21,        // 6A,fmt,PAN(10),date(2),serial(3),hashAlg,pkAlg,pkLen,expLen
                pkLenIndex = 19,
            ...
            return done(steps, rid, caIndex, "ICC public key recovery failed.")
        }
        if (!icc.formatOk) {
        ...
        // ── Step 4: recover & verify the SDAD ──
        val sdad = hexToBytes(tags.sdadHex)
        val rec = try {
            modPowFixed(sdad, iccExp, icc.modulus)
        } catch (e: Exception) {
